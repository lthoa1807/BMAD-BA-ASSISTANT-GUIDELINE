...
2. Confirm the components to be installed, including:
   - Microsoft Access Database Engine install/configuration.
   - Web service deployment or configuration.
   - System database creation.
   - Blank application database creation.
3. Click `Install`.
...
- The source document is the approved installation outline for the first draft of this guide.
...
| ID | Question |
| --- | --- |
| Q-001 | Which Windows Server versions are officially supported for this FBO installation? |
| Q-002 | Should SQL Server Authentication, Windows Authentication, or both be supported during installation? |
| Q-003 | What protocol, host name, port, and SSL/TLS requirements should IT configure for online access? |
| Q-004 | What is the official FBO installer package name and target version for this guide? |
| Q-005 | Should environment-specific values in screenshots be masked before external distribution? |
...
| --- | --- | --- | --- |
| v0.1 | 2026-05-25 | BA Assistant | Created first English draft from `FBO installation.docx`. |
